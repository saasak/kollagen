	...
	{#snippet children({ items })}
		...
		<div class="flex items-center gap-0.5">
			{#each items as item (item.index)}
				<RatingGroup.Item
					index={item.index}
					class="rounded-kl-selector text-kl-base-300 focus-visible:outline-kl-primary size-kl-selector-md flex cursor-pointer items-center justify-center transition-colors duration-150 outline-none focus-visible:outline data-[disabled]:cursor-not-allowed data-[disabled]:opacity-50 data-[readonly]:cursor-default data-[readonly]:opacity-75"
				>
					<span class="relative inline-block leading-none">
						{#if item.state === 'active'}
							<Star class="fill-kl-primary text-kl-primary size-kl-selector-sm" />
						{:else if item.state === 'partial'}
							<Star class="size-kl-selector-sm" />
